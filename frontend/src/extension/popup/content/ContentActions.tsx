...
export function ContentActions(props: {
  ...
  const tooltipText = createMemo(() => {
    if (hasFilesToDownload() && hasSomeFilesInProgress()) {
      return 'Some files are not converted yet (in progress). Only converted files will be downloaded.';
    }

    if (!hasFilesToDownload()) {
      return 'All files in the list are not available, please convert more files first';
    }
  });
  ...
  const isBatching = createMemo(
    () => batchRequestBeingCreated() !== undefined || ['queued', 'batching'].includes(batchRequest()?.status || ''),
  );
...
function getBatchRequestErrorTooltipText(errorType: BatchRequestErrorType) {
  const errorTypeToText: Record<string, string> = {
    [BatchRequestErrorType.QuotaExceeded]: `Download all files limit reached, please try again later (current limit is ${config.batchLimitPerHour} per hour)`,
  };

  return errorTypeToText[errorType] || 'Could not download all, please try again later';
}

function getBatchErrorTooltipText(_batchError: string) {
  // TODO add error codes
  return `Could not download all, please try again later`;
}
