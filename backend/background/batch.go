...
func ProcessBatchRequests(taskPool *utils.TaskPool) {
	...
	for {
		...
		if taskPool.LeftSlots() == 0 {
			logrus.Info("No available slots for new tasks, skipping fetching batch requests")
			continue
		}

		batchRequestsInProgress := make([]string, len(taskPool.OccupiedTokens()))
		for batchRequestId := range taskPool.OccupiedTokens() {
...
func createZipFromBatchRequest(batchRequestId string) error {
	...
	for _, convertRequest := range batchRequest.ConvertRequests {
		filePath := filepath.Join(utils.GetFilesDirAbsolutePath(), fmt.Sprintf("%s_converted", convertRequest.Id))
	...
	zipFilePath := filepath.Join(utils.GetFilesDirAbsolutePath(), fmt.Sprintf("%s.zip", batchRequestId))
	...
	if _, err := zipBuffer.WriteTo(zipFile); err != nil {
		return fmt.Errorf("failed to write zip buffer to file: %w", err)
	}
