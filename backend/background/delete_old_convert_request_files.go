...
func StartDeletingOldConvertRequestFiles() {
    ...
    `, thresholdMinutes)
		...
		for _, convertRequestToDeleteFile := range convertRequestsToDeleteFiles {
			uploadedFilePath := filepath.Join(utils.GetFilesDirAbsolutePath(), convertRequestToDeleteFile.Id)
			convertedFilePath := filepath.Join(utils.GetFilesDirAbsolutePath(), convertRequestToDeleteFile.Id+"_converted")
